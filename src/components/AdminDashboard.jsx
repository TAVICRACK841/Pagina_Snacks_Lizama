...
export default function AdminDashboard() {
  ...
  const markTicketAsRead = async (ticket) => {
      ...
      fetchTickets();
      showToast(newStatus === 'resolved' ? "Marcado como atendido" : "Marcado como pendiente", "info");
  };
  ...
  const handleAddExpense = async (e) => {
      e.preventDefault();
      if (!newExpense.description || !newExpense.amount || !newExpense.date) return showToast("Faltan datos", "error");
      const expenseDate = new Date(newExpense.date + 'T12:00:00');
      await addDoc(collection(db, "expenses"), { ...newExpense, amount: Number(newExpense.amount), createdAt: expenseDate.toISOString() });
  ...
  const updateTheme = async (theme) => { 
      ...
      showToast(`Tema cambiado: ${theme}`, "success"); 
  };

  const handleUpdatePhone = async () => {
      try {
          await setDoc(doc(db, "settings", "contact_info"), { phoneNumber: supportPhone }, { merge: true });
          showToast("Teléfono de soporte actualizado", "success");
      } catch (error) {
          console.error(error);
  ...
  const getRoleBadgeColor = (role) => {
      switch(role) {
          ...
          case 'hamburguesero':
          case 'freidor':
          case 'productor': return 'bg-yellow-900/50 text-yellow-300 border border-yellow-800';
          case 'mesero 1':
          case 'mesero 2':
          case 'repartidor 1':
          case 'repartidor 2': return 'bg-green-900/50 text-green-300 border border-green-800';
                  ...
                  <div className="flex flex-col gap-2">
                      ...
                      <div className="flex gap-2">
                          <input 
                              ...
                              className="border p-3 rounded w-full bg-zinc-700 text-white border-zinc-600"
                          />
        ...
        <div className="bg-zinc-800 p-4 md:p-6 rounded-lg shadow-md border border-zinc-700">
           <div className="overflow-x-auto w-full">
             <table className="w-full text-left min-w-[500px]">
               <thead><tr className="bg-zinc-700 text-yellow-400 text-xs"><th className="p-3">Usuario</th><th className="p-3">Rol Actual</th><th className="p-3">Cambiar a</th></tr></thead>
               <tbody>{users.map(u => (<tr key={u.id} className="border-b border-zinc-700 hover:bg-zinc-700"><td className="p-3"><p className="font-bold text-white">{u.displayName || 'Sin Nombre'}</p><p className="text-xs text-gray-400">{u.email}</p></td><td className="p-3"><span className={`px-2 py-1 rounded text-xs uppercase font-bold ${getRoleBadgeColor(u.role)}`}>{u.role}</span></td><td className="p-3"><select onChange={(e) => handleUpdateRole(u.id, e.target.value)} value={u.role || 'cliente'} className="border border-zinc-600 p-2 rounded text-sm bg-zinc-700 text-white w-full">{ROLES.map(rol => <option key={rol} value={rol}>{rol.toUpperCase()}</option>)}</select></td></tr>))}</tbody>
             </table>
           </div>
        </div>
      )}
