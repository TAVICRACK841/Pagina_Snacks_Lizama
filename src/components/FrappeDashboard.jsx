...
export default function FrappeDashboard() {
  ...
  const isDrinkItem = (item) => {
      ...
      const keywords = [
          ...
          'embotellado', 'coca', 'pepsi', 'sprite', 'fanta', 'sidral', 'bebida'
      ];
  ...
  useEffect(() => {
    ...
    // Usamos timestamp simple para evitar problemas de zona horaria
                    ...
                    return (
                        <div key={order.id} className={`rounded-xl border-2 shadow-xl overflow-hidden flex flex-col ${order.status === 'pendiente' ? 'bg-zinc-800 border-pink-500/50' : 'bg-pink-900/20 border-pink-500'}`}>
                            <div className={`p-3 text-white font-bold flex justify-between items-center ${order.status === 'pendiente' ? 'bg-pink-600' : 'bg-green-600'}`}>
                                ...
                                <span className="text-xs bg-black/20 px-2 py-1 rounded flex items-center gap-1"><FaClock /> {new Date(order.createdAt?.toDate ? order.createdAt.toDate() : order.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                            </div>
                            ...
                            <div className="p-3 bg-zinc-900 border-t border-zinc-700 flex flex-col gap-2">
                                {order.status === 'pendiente' ? (
                                    ...
                                    <button onClick={() => updateStatus(order.id, 'listo')} className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded-lg transition flex items-center justify-center gap-2 animate-pulse"><FaCheck /> ¡Listo para servir!</button>
                                )}
                      ...
                      <div key={order.id} className="bg-zinc-800 rounded-lg border border-zinc-700 overflow-hidden">
                          <button onClick={() => toggleExpand(order.id)} className="w-full flex justify-between items-center p-4 hover:bg-zinc-700/50 transition">
                              <div className="text-left">
                                  <p className="font-bold text-white text-sm">{order.detail}</p>
                                  <p className="text-xs text-gray-400">{new Date(order.createdAt?.toDate ? order.createdAt.toDate() : order.createdAt).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</p>
                              </div>
                              <div className="flex items-center gap-3">
                                  <span className="text-xs bg-green-900 text-green-300 px-2 py-1 rounded">Terminado</span>
                          ...
                          {expandedOrderId === order.id && (
                              <div className="bg-black/20 p-4 border-t border-zinc-700 text-sm text-gray-300">
                                  <ul className="space-y-1">
                                      {order.items.filter(isDrinkItem).map((item, i) => (
                                          <li key={i} className="flex justify-between"><span>{item.quantity}x {item.name}</span> <span>${item.price * item.quantity}</span></li>
                                      ))}
                                  </ul>
                              </div>
                          )}
